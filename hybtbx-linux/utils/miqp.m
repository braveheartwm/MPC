...
%MIQP Solve a Mixed Integer Quadratic or Linear Program (MIQP/MILP)
%
% [xmin, fmin, flag, Extendedflag] = ...
%           MIQP(H, f, A, b, Aeq, beq, vartype, lb, ub, x0, Options)
%
% solves the following MIQP
% 
%             min         0.5*x'H x + f' x
%             subject to  A   x <= b
%                         Aeq x  = beq
...
%          e04nf.m    : QP solver from NAG foundation toolbox (new version)
%          qpact.dll  : QP solver - Active set method
%          mexclp.dll : LCP solver
%
%          LP
%          --
...
%          e04mf.m    : LP solver from NAG foundation toolbox (new version)
%
...
%                    'qpact'    : QP solver - Active set method
%                    'clp'      : LCP solver
%                    (default   : 'quadprog')
...
%                    problem can be solved with qpact.dll
%                    (default   : 1e-6)
%
...
default_xxmax      = 1e7;        % specify here the default xmax for qpact
...
if isfield(Options,'solver')
    switch Options.solver
        case {'lp','linprog','lpnag','qp','quadprog','qpnag','qpact','clp'}
...
elseif strcmp(Options.solver,'qpact')
    if max(svd(H)) < matrixtol
        error('MILPs cannot be solved with qpact')
    end
end      
...
if strcmp(solver,'qpact') & (min(lb) > -inf) & (max(ub) < inf),
    xxmax = abs(max(min(lb),max(ub)))*ones(nx,1);
else
    xxmax = default_xxmax*ones(nx,1);  
end 

if strcmp(solver,'qpact'),
    if min(svd(H)) < matrixtol,
        H=H+matrixtol*eye(nx); % Add small positive def. term to Hessian
        if verbose>=1,
            warning(sprintf('Hessian close to singular, adding small positive definite perturbation %g',matrixtol));
        end
    end
    Hinv=inv(H);
end
...
while (STACKSIZE > 0)&(QPiter < maxqp)&(flag ~= -1)
    ...
    if n>0,        
        switch solver
            ...
            case 'lpnag'
                blpnag = [ subprob.beq; subprob.b];
                Alpnag = [ subprob.Aeq; subprob.A];
                bl     = subprob.lb;  
                bu     = subprob.ub; 

                N=size(subprob.Aeq,1);
                NAGsolver=lptype('nag');
                [x,clambda,how]=lpsol(subprob.f,Alpnag,blpnag,bl,bu,subprob.x0,N,verbose-1,NAGsolver);
            ...
            case 'qpnag'
                ...
                Qinv=[];
                NAGsolver=qptype('nag');
                [x,clambda,how]=qpsol(subprob.H,subprob.f,Aqpnag,bqpnag,bl,bu,subprob.x0,NAGsolver,Qinv,verbose-1);
            ...
            case 'qpact'
                ...
                xmin=-subprob.xxmax(:); % Bounds on optimizer
                a=-subprob.H*xmin;    % This is a constant term that adds to the initial basis in each QP.
                AA=[subprob.A; eye(length(subprob.f));-eye(length(subprob.f))];
                q=size(AA,1);
                rhsc=[subprob.b; subprob.ub(:);-subprob.lb(:)]-AA*xmin;
                rhsa=a-subprob.f;
                invQ=subprob.Hinv;
                TAB=[-invQ invQ*AA';AA*invQ -AA*invQ*AA'];
                basisi=[invQ*rhsa;
                    rhsc-AA*invQ*rhsa];
                ibi=-[1:n+q]';
                ili=-ibi;
                try
                    [basis,ib,il,iter]=qpact(TAB,basisi,ibi,ili,maxQPiter);
                    
                    if iter > maxQPiter,
                        warning('Maximum number of iterations exceeded, solution is unreliable. Augment Options.maxQPiter. ');
                        how='unreliable';
                    elseif iter < 0
                        how='infeasible';
                    else
                    ...
                    x=zeros(n,1);
                    warning on
                    warning('Failed to solve QP problem, maybe the feasible set is lower dimensional');
                end
            case 'clp'
                swarn = warning;
                warning off;
                options.verbose=verbose;
                [x, la, status]=clp(subprob.H,subprob.f,subprob.A,subprob.b,subprob.Aeq,...
                    subprob.beq,subprob.lb,subprob.ub,options);
                if status==0,
                    how = 'ok';
                elseif status==1,
                    how = 'infeasible';
                ...
                fval = 0.5*x'*subprob.H*x+subprob.f'*x;
                warning(swarn);                                 
            otherwise
        ...
        x=[];
        if all(subprob.b>=0) && norm(subprob.beq,'inf')<=postol, % Bug fixed April 30, 2004
            how = 'ok';
        else
            how = 'infeasible';
        end
    ...
    elseif strcmp(how,'infeasible')
        ...
        if zpi<=zstar
            ...
            if norm( round(xi) - xi,inf) < integtol
                ...
                [p0,p1,zeroOK,oneOK] = separate(subprob, branchvar,solver);
                switch method
                    case 'depth'
...
function [p0, p1, zeroOK, oneOK] = separate(prob, branchvar,solver)
if (length(prob.vartype) >= 1)
    ...
    if strcmp(solver,'qpact'),
        p0.Hinv=prob.Hinv(others, others); % inv(H11)=Hinv11
        p1.Hinv=p0.Hinv;
    end
else
