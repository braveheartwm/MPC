...
% Nicolo' Giorgetti 
% ver.0.5.9 (based on GLPK 4.2) 
% 
% 
% This document describes all control parameters currently implemented in the GLPKMEX, a 
% Matlab MEX interface for the GLPK library. Symbolic names of control parameters and 
% corresponding codes of GLPK are given on the left. Types, default values, and descriptions are 
% given on the right. 
% 
% -----------------------
...
%              Simplex iterations limit. If this value is positive, it is decreased by one each 
%              time when one simplex iteration has been performed, and reaching zero 
%              value signals the solver to stop the search. Negative value means no 
%              iterations limit. 
%
...
%              pass of the ratio test basic variables (in  the case of primal simplex) 
%              or reduced costs of non-basic variables (in the case of dual simplex) 
%              are allowed to slightly violate their bounds, but not more than 
...
%              feasible.(Do not change this parameter without detailed understanding its 
%              purpose.) 
%
...
%              Relative tolerance used to choose eligible pivotal elements ofthe simplex 
%              table.(Do not change this parameter without detailed understanding its 
%              purpose.) 
%
% objll	      type: real, default: -DBL_MAX (LPX_K_OBJLL)	
%              Lower limitof the objective function.Ifon the phase II the objective function 
%              reaches this limit and continues decreasing, the solver stops the 
%              search.(Used in the dual simplex only.) 
%
% objul	      type: real, default: +DBL_MAX (LPX_K_OBJUL)	
%              Upper limit of the objective function. If on the phase II the objective function 
%              reaches this limit and continues increasing, the solver stops the 
%              search.(Used in the dual simplex only.) 
%
% tmlim	      type: real, default: -1.0 (LPX_K_TMLIM)	
%              Searching time limit, in seconds. If this value is positive, 
%              it is decreased each time when one simplex iteration has been performed 
%              by the amount of time spent for the iteration, and reaching zero 
%              value signals the solver to stop the search. Negative value means no time limit. 
%
...
%              Relative tolerance used to check if the value of the objective function is not 
%              better than in the best known integer feasible solution. (Do not change this 
