function result = subsref(lincon,Struct)
%SUBSREF  Subscripted reference for lincon objects.
%
%   The following reference operation can be applied to any 
%   lincon object: 
%      lincon.Fieldname  equivalent to GET(lincon,'Fieldname')
...
% Effect on lincon properties: all inherited
...
case '.'
   ...
   try
      if StructL==1,
         result = get(lincon,Struct(1).subs);   
      else
         %Struct(2).subs=names(Struct(1).subs,Struct(2).subs);
         result = subsref(get(lincon,Struct(1).subs),Struct(2:end));
      end
   catch
      error(lasterr)
   end
otherwise
   error('lincon:subsref:ref',['Unknown reference type: ' Struct(1).type])
end
